/* config-ohos.h — OHOS config baseline for full Emacs core port (Stage 3).
 *
 * What this is:
 *   Copy of /tmp/ohos-config-probe3.h (3483-line generated src/config.h from
 *   upstream /tmp/ohemacs-src/emacs-30.1), checked in as the starting baseline
 *   for cross-configuring the full Emacs engine for OpenHarmony (aarch64 musl).
 *   Generated with:
 ...
 *   Recorded as: EMACS_CONFIG_OPTIONS in this file.
 *   Upstream src/config.h is NOT edited in-tree; this copy lives outside the
 *   Emacs tree so the OHEmacs HAP build (Stage 1/2) stays intact. It is
 *   header-only and is NOT referenced by entry/src/main/cpp/CMakeLists.txt.
 *
 * Workarounds baked into this baseline (do not drop until real fixes land):
 *   1. musl-alias for missing ohos triplet: Autoconf config.sub in Emacs 30.1
 *      does not recognise *-ohos / *-openharmony, so configure was run with
 *      --host=aarch64-unknown-linux-musl as an alias. Result: GNU_LINUX and
 *      MUSL_LIBC are defined, but no HAVE_OHOS exists yet. A real
 *      XCONFIGURE=ohos / configure.ac patch must add an `ohos' OS case
 *      (see README.md) and then this file must be regenerated with the true
 *      triplet.
 *   2. termcap stub: OHOS musl 1.2.0 sysroot + SDK has no ncurses/tinfo.
 *      /tmp/ohemacs-stub/termcap-stub.c provides BC/UP/PC/ospeed +
 *      tputs/tgetent/tgetflag/tgetnum/tgetstr/tgoto, archived as
 *      libtermcap.a + alias libs (libcurses.a, libncurses.a, libterminfo.a,
 *      libtinfo.a) with LDFLAGS=-L/tmp/ohemacs-stub so configure's terminal
 *      link probes pass. This yields TERMINFO=1 / TERMINFO_DEFINES_BC=1 here.
 *      It is link-only scaffolding, not a terminal implementation.
 *   3. PATH diff-shadowing fix: configure shells out to `diff' (and cmp/sed)
 *      for feature probes. The probe only completed once PATH was fixed so
 *      the host /usr/bin/diff is found first, ahead of any SDK/stub shadowing
 *      copy. If re-running configure, use e.g.
 *        PATH=/usr/bin:/bin:$PATH ./configure ...
 *      and confirm `command -v diff` points at the host GNU/BSD diff before
 *      trusting a regenerated config.h.
 *
 * SDK gaps visible in this file (all expected with --without-all on musl 1.2.0):
 *   HAVE_JANSSON/GMP/ZLIB/GNUTLS/SQLITE3/LIBXML2/FREETYPE/HARFBUZZ/CAIRO/X11/
 *   MODULES/NATIVE_COMP/PDUMPER/UNEXEC all #undef; THREADS_ENABLED #undef
 *   (HAVE_PTHREAD* still 1); HAVE_STDBIT_H / HAVE_STDCKDINT_H #undef (musl
 *   1.2.0 lacks <stdbit.h>/<stdckdint.h>); HAVE_ANDROID/NS/W32/HAIKU/PGTK all
 *   #undef. Positive signals kept: HAVE_MMAP, HAVE_PSELECT, HAVE_TIMERFD,
 *   HAVE_TIMER_SETTIME, epoll/eventfd/signalfd family, sys/socket+select+time.
 *
 * TODOs (must resolve before production engine):
 *   TODO(HAVE_OHOS): patch configure.ac + src/*.h to define HAVE_OHOS (model
 *     on HAVE_ANDROID in androidterm.c/androidfns.c/frame.h/dispextern.h/
 *     termhooks.h/keyboard.c), add FRAME_OHOS_P + ohos display backend hooks,
 *     then regenerate this header with the real --host=aarch64-*-ohos*.
 *   TODO(ncurses port): replace termcap stub with a real ncurses/terminfo
 *     port or a fuller terminfo stub + --without-gpm; until then TTY code
 *     paths needing tputs/tgoto only link, they do not run correctly.
 *   TODO(gnulib stdbit): musl 1.2.0 lacks <stdbit.h>/<stdckdint.h>; either
 *     backport gnulib stdbit/stdckdint modules or bump to a sysroot that
 *     provides them, else lib/ probes keep HAVE_STDBIT_H undef and code
 *     needing C23 bit ops needs gnulib fallbacks.
 *   TODO(threads/pdumper): this baseline is --without-threads +
 *     --with-dumping=none (HAVE_PDUMPER/HAVE_UNEXEC undef). Re-enable threads
 *     then pdumper step-wise per README.md; do not flip these defines by hand.
 *
 * Usage: reference only for the out-of-tree Stage 3 cross-build
 *   (XCONFIGURE=ohos plan). Do NOT #include this from Stage 1/2 NAPI sources;
 *   keep it out of CMakeLists.txt until the full-engine libemacs.so build
 *   consumes it as src/config.h.
 */
...
#define EMACS_CONFIGURATION "aarch64-unknown-linux-musl"

/* Summary of some of the main features enabled by configure. */
#define EMACS_CONFIG_FEATURES "SECCOMP"

/* Define to the options passed to configure. */
#define EMACS_CONFIG_OPTIONS "--host=aarch64-unknown-linux-musl --without-all --with-dumping=none --without-threads --without-modules 'CFLAGS=-DOHOS_PLATFORM -D__MUSL__' LDFLAGS=-L/tmp/ohemacs-stub"
